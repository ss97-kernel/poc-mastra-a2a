...
import {
  DATA_PROCESSOR_TASK_WORKFLOW_ID,
  dataProcessingTaskWorkflowInputSchema,
} from '../mastra/workflows/taskProcessorWorkflow.js';
...
function buildWorkingTask(taskId: string, text = 'Processing data task...') {
  return {
    task: {
      id: taskId,
...
async function startGovernedTask(taskData: any, taskId: string) {
  const workflow = mastra.getWorkflow(DATA_PROCESSOR_TASK_WORKFLOW_ID);
  if (!workflow) {
    throw new Error(`Workflow ${DATA_PROCESSOR_TASK_WORKFLOW_ID} is not registered`);
  }

  const run = await workflow.createRun();
  const workflowInput = dataProcessingTaskWorkflowInputSchema.parse({
    ...taskData,
    taskId,
  });
  ...
  if (
    ...
    throw new Error(
      `Workflow ${DATA_PROCESSOR_TASK_WORKFLOW_ID} failed with status ${result.status}`
    );
  }
...
async function resumePendingTask(taskId: string) {
  ...
  try {
    ...
    if (
      ...
      throw new Error(
        `Workflow ${DATA_PROCESSOR_TASK_WORKFLOW_ID} failed with status ${resumed.status}`
      );
    }
